...
public class BoardServiceImpl implements BoardService {
    ...
    public BoardDTO findById(Long boardNo) {
        Board getBoard = boardRepository.findById(boardNo).orElseThrow();

        return getBoard.toDTO();
    }
    ...
    public void modify(BoardDTO boardDTO, List<BoardFileDTO> uBoardFileList) {
        ...
        List<BoardFile> boardFileList = uBoardFileList.stream().map(uBoardFile -> uBoardFile.toEntity(board)).toList();

        boardFileList.stream().forEach(
                boardFile -> {
                    if(boardFile.getBoardFileStatus().equalsIgnoreCase("U") || boardFile.getBoardFileStatus().equalsIgnoreCase("I")){
